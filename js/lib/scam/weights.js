// Mirrors src/rithik/scam/weights.py. Keep the two in step: the golden tests fail otherwise.
//
// Every tunable number in the scam checker lives here, so tuning never means hunting through
// regexes. A key is either a reason code ("otp_request") or a code plus a variant
// ("account_blocked.past"). The variant lets one reason fire at different strengths without
// inventing new codes that JSON consumers would have to learn; the reason that reaches the user
// always carries the bare code.
...
/** WEIGHTS[key], failing loudly on an unknown key as Python's dict lookup does. */
export function weightOf(key) {
  if (!Object.hasOwn(WEIGHTS, key)) throw new Error(`unknown weight key: ${key}`);
  return WEIGHTS[key];
}
...
export const SUSPICIOUS_THRESHOLD = 0.25;
